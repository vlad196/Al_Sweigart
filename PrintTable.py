...
def printtable(data):  # Создаём функцию, где:
    ...
    rows = max(map(len, data))  # Число строк это кол-во объектов в 1-м списке списка data (неуниверсально.
    ...
    for col in data:  # создаём цикл значений col из data
        col_wigth.append(len(sorted(col, key=len)[-1]))  # В спискок col_wight вставляем количество символов
        # последнего объекта (col) из списка [-1] отсортированного по длине символов sorted key = len
